...
func (board *kanbanBoard) KanbanToolDefs() []kanbanToolDef {
	...
	return []kanbanToolDef{
		...
		{
			Name:        "confirm_action",
			Description: "Execute pending medium/high-risk Jira actions only after the live user explicitly confirms them. When confirmation_id is omitted, confirm all active pending actions together.",
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"confirmation_id": map[string]any{"type": "string", "description": "Confirmation id returned by the pending action. Leave empty when the user gives a simple yes that confirms every active pending action."},
				},
				"additionalProperties": false,
			},
		},
