...
func (board *kanbanBoard) NovaSonicSessionInstructions() string {
	return strings.Join([]string{
		...
		"If a tool returns requires_confirmation, read its prompt and wait for live confirmation, then call confirm_action. If multiple requested actions are pending and the user gives one yes/no confirmation for all of them, call confirm_action or cancel_confirmation once with no confirmation_id so the whole pending set resolves together. If confirm_action returns requires_disambiguation, relay its prompt to the live user and wait for their answer; do not self-confirm a mixed-risk set with an aggregate phrase. If declined, call cancel_confirmation.",
...
func (board *kanbanBoard) KanbanToolDefs() []kanbanToolDef {
	...
	return []kanbanToolDef{
		...
		{
			Name:        "confirm_action",
			Description: "Execute pending medium/high-risk Jira actions only after the live user explicitly confirms them. When confirmation_id is omitted, confirm all active pending actions together, except when the pending actions are at different risk levels: a bare yes then returns requires_disambiguation and you must ask the live user which to confirm rather than sweeping them.",
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"confirmation_id": map[string]any{"type": "string", "description": "Confirmation id returned by the pending action. Leave empty when the user gives a simple yes that confirms every active pending action. If the pending actions span different risk levels, a bare yes is rejected with requires_disambiguation; pass a specific id, or an explicit aggregate phrase the live user actually said (such as 'yes to all'), never an aggregate phrase you chose yourself."},
				},
				"additionalProperties": false,
			},
		},
		{
			...
			Description: "Cancel pending medium/high-risk Jira actions when the user says no, cancel, never mind, or stop. When confirmation_id is omitted, cancel all active pending actions together.",
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"confirmation_id": map[string]any{"type": "string", "description": "Confirmation id to cancel. Leave empty when the user gives a simple no that cancels every active pending action."},
				},
				"additionalProperties": false,
			},
		},
