...
func (board *kanbanBoard) SessionInstructions() string {
	return strings.Join([]string{
		...
		"If a participant speaks or types in a non-English language, every assistant message for that participant turn MUST be self-contained bilingual: first acknowledge or answer in that participant's language or dialect, then say 'For the room:' and provide the English meaning or outcome. In audio/video meetings, speak the 'For the room:' English portion out loud in the same response so other participants understand. If the response is split into multiple assistant messages, repeat this bilingual pattern in every message. Apply this to direct replies, tool-result confirmations, and confirmation prompts. Never continue in any non-English-only mode after a non-English participant message. Never send English-only follow-up fragments after a non-English participant message. Use English for all board, Jira, GitHub, recap, meeting-memory, and tool-call fields.",
