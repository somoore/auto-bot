...
- Go server to SQLite/EFS/local disk.
- Terraform/Terragrunt to AWS APIs.
...
| Stolen local token or browser session | Unauthorized meeting or board access | HttpOnly cookies, no token in served HTML, local-only login token, planned OIDC/Cognito for public deployment |
...
| AWS secret leakage | Cloud compromise | Secrets Manager for production secrets, no `.env`, private subnets, least-privilege IAM, WAF, no NAT Gateway dependency |
...
- Public multi-tenant hosting needs OIDC/Cognito, workspace-scoped secrets, and per-connector install records.
